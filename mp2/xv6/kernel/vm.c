...
void uvmlazytouch(uint64 va) {
	...
	if(mem == 0) {
		...
		if(*pte & PTE_S){
			begin_op();
			char* page = kalloc();
			read_page_from_disk(ROOTDEV, page, PTE2BLOCKNO(*pte));
...
void _pteprint(pagetable_t pagetable, int level, int print[2], uint64 va) {
	int MAX = 0;
	for(int i = 0; i < 512; i++)
	...
	for(int i = 0; i < 512; i++) {
		...
		if (pte & (PTE_V | PTE_S)) {
			for (int j = 0; j <= level; j++){
				if(j == level) printf("+-- ");
				else if(print[j]) printf("|   ");
				else printf("    ");
			}
			if(pte & PTE_S) pte &= ~PTE_V;
			printf("%d: pte=%p va=%p %s=%p", i, pagetable + i, va + ((uint64)i << (12 + 9 * (2 - level))),\
				   	(pte & PTE_S) ? "blockno" : "pa", (pte & PTE_S) ? (uint64)PTE2BLOCKNO(pte) : PTE2PA(pte));
		...
		if ((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_W)) == 0) {
			uint64 child = PTE2PA(pte);
			int t = print[level];
			if(i < MAX) print[level] = 1;
			_pteprint((pagetable_t)child, level+1, print, va + ((uint64)i << (12 + 9 * (2 - level))));
			print[level] = t;
		}
	}
}
...
int madvise(uint64 base, uint64 len, int advice) {
	...
	if(advice == MADV_NORMAL){
		...
		begin_op();
		for(uint64 i = PGROUNDDOWN(base), j = PGROUNDUP(base + len)/* - 4096 * (PGROUNDUP(base + len) == PGROUNDDOWN(base + len))*/; i < j; i += 4096){
			pte_t *pte = walk(p->pagetable, i, 0);
			if(pte == 0) continue;
			if(*pte & PTE_V){
				uint blk = balloc_page(ROOTDEV);	
				write_page_to_disk(ROOTDEV, (char*)PTE2PA(*pte), blk);
				kfree((void*)PTE2PA(*pte));
				*pte = BLOCKNO2PTE(blk) | PTE_FLAGS(*pte);
				*pte &= ~PTE_V;
				*pte |= PTE_S;
			}
		}
		...
		for(uint64 i = PGROUNDDOWN(base), j = PGROUNDUP(base + len)/* - 4096 * (PGROUNDUP(base + len) == PGROUNDDOWN(base + len))*/; i < j; i += 4096){
			pte_t *pte = walk(p->pagetable, i, 0);
			if(!(*pte & (PTE_S | PTE_V)) && uvmshouldtouch(i)){
				uvmlazytouch(i);
			} else if(*pte & PTE_S) {
				char* page = kalloc();
				read_page_from_disk(ROOTDEV, page, PTE2BLOCKNO(*pte));
